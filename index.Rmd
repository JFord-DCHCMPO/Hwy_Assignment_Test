...
p <- ggplot(data=joined, aes(x=reorder(County, -temp), y=temp, fill = Output)) +geom_bar(stat='identity', position='dodge') + theme_classic() + xlab('')+ ylab('')+ ggtitle("County Level Totals")+ theme(axis.text.x=element_text(angle = -45, hjust = 0))+ scale_y_continuous(labels=comma)+ labs(fill = "")
...
Overview {.storyboard data-icon="ion-stats-bars"}
=====================================  
...
Placeholder page for defining acronyms - VMT, VHT, VOC

TRMG2 classifies sub-areas as clusters in the following design: 

TRMG2  Cluster Map: ![](ClusterMap.png) 
